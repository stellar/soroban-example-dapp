...
import { Campaign, Pledge } from '../components/molecules'
    ...
    balance: useContractValue(
      ...
      xdr.ScVal.scvObject(
        xdr.ScObject.scoVec([
          ...
          xdr.ScVal.scvObject(
            xdr.ScObject.scoBytes(Buffer.from(CROWDFUND_ID, 'hex'))
          ),
        ])
      )
    ),
  ...
  const yourDepositsXdr = useContractValue(
    ...
    xdr.ScVal.scvObject(
      xdr.ScObject.scoVec([
        xdr.ScVal.scvSymbol('Account'),
        xdr.ScVal.scvObject(
          account ? xdr.ScObject.scoBytes(Buffer.from(account.address)) : null
        ),
      ])
    )
  )
...
function DepositForm({
  ...
  const user = xdr.ScVal.scvObject(
    xdr.ScObject.scoVec([
      xdr.ScVal.scvSymbol('Account'),
      // TODO: Parse this as an address or whatever.
      xdr.ScVal.scvObject(
        xdr.ScObject.scoBytes(Buffer.from(account.address, 'hex'))
      ),
    ])
  )
  ...
  return (
    <form
      onSubmit={async e => {
        ...
        let accountKey = SorobanSdk.StrKey.decodeEd25519PublicKey(
          account.address
        )
